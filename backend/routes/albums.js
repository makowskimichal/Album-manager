...
router.post('/favorites', async (req, res) => {
    ...
    if (!oldAlbum) {
        ...
        return res.send({
            message: `You have already added ${oldAlbum.albumName} to favorites`,
        })
    } else if (oldAlbum.isFavorite === false) {
        return res.send({
            message: `Album ${oldAlbum.albumName} has been edited`,
            album: await Album.findOneAndUpdate(
                { imageUrl: req.body.data.imageUrl, username: req.body.user },
                { isFavorite: true, lastUpdated: Date.now() }
            ),
        })
    }
})
...
router.post('/bought', async (req, res) => {
    ...
    if (!oldAlbum) {
        ...
        oldAlbum.isBought === true &&
        JSON.stringify(req.body.data.boughtMedium) ===
            JSON.stringify(oldAlbum.boughtMedium)
    ) {
        return res.send({
            message: `You have already purchased ${oldAlbum.albumName}`,
        })
    } else if (
        oldAlbum.isBought === true &&
        JSON.stringify(req.body.data.boughtMedium) !==
...
router.post('/wishlist', async (req, res) => {
    ...
    if (!oldAlbum) {
        ...
        return res.send({
            message: `Album ${oldAlbum.albumName} is already on the wishlist`,
        })
        ...
        return res.send({
            ...
            album: await Album.findOneAndUpdate(
                { imageUrl: req.body.data.imageUrl, username: req.body.user },
                { isWishlist: true, lastUpdated: Date.now() }
            ),
        })
    }
})
...
router.get('/recommend', async (req, res) => {
    Album.find({ username: req.query.user })
        .sort({ lastUpdated: -1 })
        .limit(5)
        .exec((err, album) => {
            if (err) {
            ...
            const array = []
            spotifyApi
                .getRecommendations({ limit: 5, seed_artists: arraySeeds })
                .then(
                    function (data) {
...
// search for albums in bought section
...
// search for albums in favorites section
...
// search for albums in wishlist section
...
// return all the bought albums for timeline sorted by date of purchase
